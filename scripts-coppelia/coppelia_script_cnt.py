...
while True:
	...
	try:
		...
		if flag == 10:
			...
			# 3D coordinates
			R_shoulder = [round(R_shoulder_2D[0], 3), round(R_shoulder_2D[1], 3), R_hip_depth]
			...
			# ------------------------------------- Angle calculation -------------------------------------
			# Neck
			Head_pitch = - calculate_angle((Neck[1], Neck[2]),(Aux[1], Aux[2]),(Goggles[1], Goggles[2])) + 135
			...
			if (L_elbow[0] > L_shoulder[0] and L_elbow[1] < L_shoulder[1]):
				if (L_wrist[0] < L_elbow[0] and L_wrist[1] < L_elbow[1]): 
					L_elbow_yaw = - (calculate_angle((L_elbow[1], L_elbow[2]-20), (L_elbow[1], L_elbow[2]), (L_wrist[1], L_wrist[2]))*1.1)
			...
			if np.sum(avg_angle) == 0:
				...
				avg_Head_yaw = clamp(avg_Head_yaw, -55, 55)
				avg_Head_pitch = clamp(avg_Head_pitch, -32, 32)
				joints = [avg_R_shoulder_pitch]
				rightarm_joints = [avg_R_shoulder_roll, avg_R_shoulder_pitch, avg_R_elbow_roll, 28, avg_R_wrist_yaw, close_right]
				...
				#rightarm_joints = [0, 0, 0, 0, 0, 0]
				#leftarm_joints = [0, 0, 0, 0, 0, 0]
				head_joints = [0, 0]
	...
	#print(cX, "\n ---------- \n", cY, "\n ---------- \n", round(aligned_depth_frame.get_distance(int(cX),int(cY))*100), "\n ---------- \n",)
	#print(R_shoulder_depth,"\n ---------- \n", R_hip_depth,"\n ---------- \n", R_elbow_depth)
	print(joints)
