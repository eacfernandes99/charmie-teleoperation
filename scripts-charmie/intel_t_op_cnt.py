...
while(True):
    ...
    try:
        ...
        R_shoulder_2D = [landmarks[mp_holistic.PoseLandmark.RIGHT_SHOULDER.value].x * 640,
				         landmarks[mp_holistic.PoseLandmark.RIGHT_SHOULDER.value].y * 480]
        Neck_2D = [(R_shoulder_2D[0]+L_shoulder_2D[0])/2, 
                   (R_shoulder_2D[1]+L_shoulder_2D[1])/2]
        Aux_2D = [round((R_shoulder_2D[0]+L_shoulder_2D[0])/2, 0), 
                  round(Neck_2D[1] - np.sqrt((R_shoulder_2D[0]-L_shoulder_2D[0])**2+(R_shoulder_2D[1]-L_shoulder_2D[1])**2)/2, 0)]
        ...
        if flag < 6:
            ...
            depth_values = [L_shoulder_depth, R_shoulder_depth, L_elbow_depth, L_wrist_depth, L_shoulder_depth, L_middle_tip_depth]
            print(depth_values)
            for i in range(len(depth_values)):
                if 50< depth_values[i] < 110:
                    flag = flag + 1
        if flag == 6 and flag_contour == 1:
            print("Flag enabled")
            ...
            c_depth_values = [cL_shoulder_depth, cR_shoulder_depth, cL_elbow_depth, cL_wrist_depth, cL_shoulder_depth, cL_middle_tip_depth]
            ...
            Neck = [round(Neck_2D[0], 3), round(Neck_2D[1], 3), (R_shoulder_depth + L_shoulder_depth)/2]
            Aux = [Aux_2D[0], Aux_2D[1], (R_shoulder_depth + L_shoulder_depth)/2]
            Goggles = [cX, cY, round(aligned_depth_frame.get_distance(int(cX),int(cY))*100), 0]

            # ------------------------------------- Angle calculation -------------------------------------
			
            # Neck
            Head_pitch = calculate_angle((Neck[1], Neck[2]),(Aux[1], Aux[2]),(Goggles[1], Goggles[2]))
            Head_yaw = calculate_angle((Aux[0], Aux[2]-10),(Aux[0], Aux[2]),(Goggles[0], Goggles[2]))
            if(Goggles[0]<Aux[0]):
                Head_yaw = - Head_yaw
            ...
            # Clamping values
            Head_yaw = clamp(Head_yaw, -75, 15)
            Head_pitch = clamp(Head_pitch, 100, 170)
            ...
            Head_yaw_f= int(convert(Head_yaw, -75, 15, 1600, 2600))
            Head_pitch_f = int(convert(Head_pitch, 100, 170, 1600, 2600))
            # 2100
            # 2100            

            servo_values = [L_shoulder_roll_f, L_shoulder_pitch_f, L_elbow_yaw_f, L_elbow_roll_f, close_left, Head_yaw_f, Head_pitch_f]
            #servo_values = [close_pos_arm[0], close_pos_arm[1], close_pos_arm[2], open_pos_arm[3], 0, Head_yaw_f, Head_pitch_f]
            #servo_values = [L_shoulder_roll_f, L_shoulder_pitch_f, L_elbow_yaw_f, L_elbow_roll_f, close_left, 2100, 2100]
